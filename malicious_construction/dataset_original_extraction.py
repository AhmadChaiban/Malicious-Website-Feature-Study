...
dataset = pd.read_csv('./2 - custom_features_data/benign_new_features_5.csv')
...
# dataset.to_csv('./3 - separate_pending_data/batch_5/benign_https.csv', index=False)
...
# dataset.to_csv('./3 - separate_pending_data/batch_5/benign_tld_urllen.csv', index=False)
# print(dataset['tld'].value_counts())

# dataset['who_is'] = dataset['url'].progress_apply(lambda x: get_who_is(x))
# dataset.to_csv('./3 - separate_pending_data/batch_5/benign_whois.csv', index=False)
# print(dataset['who_is'].value_counts())

dataset['content'] = dataset['url'].progress_apply(lambda x: get_url_content(x))
dataset.to_csv('./3 - separate_pending_data/batch_5/benign_content.csv', index=False)
